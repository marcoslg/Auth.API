...
namespace Authorize.Identity.WebAPI.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        {
            var users = await SearchByProperties(search);// { Limit = limit, Offset = offset });
                        

            return ResponseGet(users);
        }

        private async Task<IEnumerable<ApplicationUser>> SearchByProperties(string query)//, IQuerySort sort, IQueryPaging paging)
        {
            var users = await _userManager.Users.Where(user => user.UserName.Contains(query) || user.Email.Contains(query)).ToListAsync()
